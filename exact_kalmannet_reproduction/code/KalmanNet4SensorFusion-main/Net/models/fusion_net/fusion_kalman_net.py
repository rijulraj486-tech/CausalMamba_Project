...
class FusionKalmanNet(KalmanNet):
    ...
    def step_prior(self, m1x_posterior: torch.Tensor,
        ...
        bad = (~torch.isfinite(m1x_prior).flatten(1).all(dim=1)) | (
            ~torch.isfinite(m1y).flatten(1).all(dim=1))
        if torch.any(bad):
            fallback_state = torch.nan_to_num(
                self.m1x_posterior.detach().clone(),
                nan=0.0,
                posinf=1e6,
                neginf=-1e6,
            )
            fallback_obs = torch.nan_to_num(
                self.y_previous.detach().clone(),
                nan=0.0,
                posinf=1e6,
                neginf=-1e6,
            )
            m1x_prior[bad] = fallback_state[bad]
            m1y[bad] = fallback_obs[bad]
